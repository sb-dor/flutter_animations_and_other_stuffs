...
class MainGoogleMapCubit extends Cubit<GoogleMapStates> {
  ...
  Future<void> _createMarkerOnCenter(
      GoogleMapController controller, BuildContext context) async {
  ...
  void _changeCenterMarker(CameraPosition cameraPosition) {
    ...
    Marker newMarker =
        centerMarker.copyWith(positionParam: cameraPosition.target);
  ...
  Future<void> _findAddressByCoordinatesOnTap(LatLng latLng) async {
    ...
    final address = await geocode.placemarkFromCoordinates(
  ...
  Future<void> getApproximateTimeOfUserLocation() async {
    _currentState.serviceEnabled =
        await _currentState.location.serviceEnabled();
    if (!_currentState.serviceEnabled) {
      _currentState.serviceEnabled =
          await _currentState.location.requestService();
    }
    ...
    bool isBackModeEnable =
        await _currentState.location.isBackgroundModeEnabled();
    ...
    await for (var each in _currentState.location.onLocationChanged) {
      debugPrint(
          "each listening position: ${each.latitude} | ${each.longitude}");
  ...
  void _clearDestinationsMarks() {
    _currentState.markers.removeWhere((key, value) =>
        key.value == "destination_1" || key.value == 'destination_2');
    _currentState.polyLines
        .removeWhere((key, value) => key.value == 'destination');
    _currentState.polyLineDestinationIds = 0;
  }

  void startToSelectDestination() {
    _currentState.selectingUserDestination =
        !_currentState.selectingUserDestination;
  ...
  void addTwoPolyLinesBetweenTwoDestinations(LatLng latLng) async {
    ...
    MarkerId id =
        MarkerId("destination_${_currentState.polyLineDestinationIds}");
    Marker marker = Marker(
      markerId: id,
    ...
    final Marker? firstMarker =
        _currentState.markers[const MarkerId("destination_1")];
    final Marker? secondMarker =
        _currentState.markers[const MarkerId("destination_2")];
    debugPrint(
        "first: ${firstMarker?.position} | second: ${secondMarker?.position}");
      ...
      PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
        ...
        request: PolylineRequest(
          ...
          destination: PointLatLng(
              secondMarker.position.latitude, secondMarker.position.longitude),
